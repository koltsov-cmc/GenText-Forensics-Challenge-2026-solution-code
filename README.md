...
An example of a DTD model in action. From left to right: a fragment of a clean document, a heatmap of probabilities from the DTD, bboxes derived from connected components, a ground truth mask.
...
| `--filterer_prompt` | — | Filterer template (`{{DTD_HINTS}}`). |
| `--semantic_detective_prompt` | — | Detective template (`{{OCR_JSON}}`, `{{FILTERED_DTD}}`). |
